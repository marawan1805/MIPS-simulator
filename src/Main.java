...
public class Main {
	...
	public void runInstruction(String instruction) {
		...
		if (parts[0].equals("ADD")) {
			...
			String[] operands = parts[1].split(",");
			String[] s1 = operands[1].split("\\(|\\)");
			int indexOutput = getRegisterIndex(operands[0]);
			int indexMemory = getRegisterIndex(s1[1].substring(s1[1].length()));
			int offset =  Integer.parseInt(s1[0])/4;
			
			register[indexOutput] = memory[indexMemory + offset];
			printNonEmpty(register);
			System.out.println("--------Instruction Done---------");
		    ...
		    memory[indexMemory + offset] = register[indexOutput];
	...
	public void runFromMiddle(String instruction){
		
		String[] parts = instruction.split(" ");
		String[] operands = parts[1].split(",");
		
		int indexOp0 = getRegisterIndex(operands[0]);
		int indexOp1 = getRegisterIndex(operands[1]);
		String toBeCompared = operands[2];
			
		if(register[indexOp0] == register[indexOp1]){
			try {
				...
				while (myReader.hasNextLine()) {
					...
					if(parts1[0].equals(toBeCompared + ":")){
